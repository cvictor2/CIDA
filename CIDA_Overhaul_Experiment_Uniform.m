function [nu_nodes,all_min_nodes] = CIDA_Overhaul_Experiment_Uniform(mu_input,type)
...
% nu_nodes = [7.5e-6];
all_min_nodes = zeros(length(nu_nodes),retrials);
seeds = all_min_nodes;
for(run = 1:trials)
...
%                     param.t_adj = t - t_ramp;
        ...
        if(save_check)
            ...
            save(sprintf('Data/Overhauled/%s_Uniform_Trials_nu(varies)_alpha=%.0d_mu=%.0d.mat',date_string,alpha,mu),'param','vars','all_min_nodes','nu_nodes','seeds');
        end
    end
end
if(vars(1).type) == 1
...
save(sprintf('Results/%s_Uniform_Trials_type=%s_alpha=%.0d_mu=%.0d_finished.mat',date_string,ttype,alpha,mu),'param','vars','all_min_nodes','nu_nodes','seeds');
end
