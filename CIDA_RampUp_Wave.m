...
u = u_0';
t=0;
%% Sweeping Data Assimilation

% mu = 50;
nodes = min_nodes; %number of nodes
vel = nodes;
int_nodes = nodes;
...
for( k = 1:timesteps-offset)
    ...
    [x_nodes, i_nodes] = updateNodes(i_nodes,v);
...
% i_nodes =rem((prev_nodes+velocity),N);
% if(i==0)
i_nodes = (2 + mod(prev_nodes+velocity-1, N-2));
% i_nodes = prev_nodes;
% i_nodes = mod(prev_nodes+velocity,N);
% for i = 1:length(prev_nodes)
i_nodes = sort(i_nodes);
x_nodes = x(i_nodes);
