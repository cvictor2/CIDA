...
%             end
    ...
    if(car)
        if(k~=1)
            [x_nodes_c, i_nodes_c] = updateNodes(x_nodes_c, t-t_ramp);
        end
        umv2 = u - v_c;
        %         Ih_umv = interpl(x_nodes_c,umv2(i_nodes_c),
        [maximum,index] = max(x_nodes_c(2:end)-x_nodes_c(1:end-1));
        if(maximum > dx*length(x_nodes_c))
%             x_nodes(1:index) = x_nodes(1:index)+x(2);
            Ih_umv_c = [zeros(1,i_nodes_c(1)-1),interp1(x_nodes_c,umv2(i_nodes_c(1:index)),x(i_nodes_c(1:index))),  zeros(1,(i_nodes_c(index):i_nodes_c(index+1))), interp1(x_nodes_c,umv2(i_nodes_c(index+1:end)),x(i_nodes_c(index+1:end))), zeros(1,N-i_nodes_c(end))]';
...
function [x_nodes, i_nodes] = updateNodes(prev_nodes, t)
...
%     x_nodes = x(i_nodes);
%     
%     i_nodes = find(([x,x+1]>=mod(prev_nodes(1) + velocity*t,2))&([x,x+1]<mod(prev_nodes(1) + velocity*t + length(prev_nodes)*x(2),2)));
    x_nodes = unique(floor(mod(prev_nodes + velocity*t,1)./x(2)).*x(2));
    [maximum,index] = max(x_nodes(2:end)-x_nodes(1:end-1));
    if(maximum > x(2)*length(x_nodes))
        x_nodes(1:index) = x_nodes(1:index)+x(2);
        i_nodes = x_nodes./x(2);
        i_nodes(1:index) = i_nodes(1:index)+1;
    else
        i_nodes = x_nodes./x(2);
    end
%     x_nodes = x(i_nodes);
